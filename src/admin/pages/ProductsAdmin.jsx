                ...
                <div>
                  ...
                  <div className="cms-form-group" style={{ marginBottom:10 }}>
                    ...
                    <input value={form.shortDesc||''} onChange={e=>set('shortDesc',e.target.value)}
                      placeholder="e.g. Oil, fuel, and water resistance — optimized for various applications."
                      style={{ fontSize:13 }} />
                  </div>
                  ...
                  <div className="cms-form-group" style={{ marginBottom:14 }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      <input type="color" value={form.color||'#e74c3c'} onChange={e=>set('color',e.target.value)}
...
function AutomobileProductsAdmin() {
  ...
  const categories = state.products.autoCategories || [];
  const parts      = state.products.parts           || [];

  return (
    <div>
      <div style={{ background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:10,
        padding:'11px 15px', marginBottom:18, fontSize:13, color:'#1e40af',
        display:'flex', alignItems:'flex-start', gap:9 }}>
        <span style={{ flexShrink:0 }}>ℹ️</span>
        <span>
          The car image and pin positions are fixed. You can <strong>rename any part</strong>,
          edit its description, and <strong>edit each category's name, colors, and descriptions</strong>
          — changes appear instantly on the public Products page.
        </span>
      </div>
      ...
      {tab === 'categories' && (
        <div className="cms-card">
          <div className="cms-card-title" style={{ marginBottom:14 }}>
            Automobile Categories — edit name, color, and descriptions
          </div>
          <CategoryEditor
            categories={categories}
            parts={parts}
            onUpdateCat={(cat) => dispatch({ type:'AUTO_CAT_UPDATE', payload:cat })}
          />
        </div>
      )}
      ...
      {tab === 'parts' && (
        <div className="cms-card">
          <div className="cms-card-title" style={{ marginBottom:14 }}>
            Automobile Parts — click Edit to rename or update description
          </div>
          <PartsEditor
            categories={categories}
            parts={parts}
            onUpdatePart={(pt) => dispatch({ type:'PART_UPDATE', payload:pt })}
          />
        </div>
      )}
    </div>
    ...
    <div>
      <div className="cms-page-header">
        <div>
          ...
          <p className="cms-page-sub">Edit categories and part names — changes show instantly on the website</p>
        </div>
      </div>
      <div className="cms-tab-bar">
        <button className={`cms-tab${tab==='automobile'?' active':''}`} onClick={()=>setTab('automobile')}>
          🚗 Automobile Products
        </button>
        <button className={`cms-tab${tab==='motorcycle'?' active':''}`} onClick={()=>setTab('motorcycle')}>
          🏍 Motorcycle Products
        </button>
      </div>
